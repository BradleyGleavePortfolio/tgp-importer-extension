...
const DEFAULT_LIMITS = Object.freeze({ maxEntries: 1000, maxTotalBytes: 8 * 1024 * 1024, maxBodyBytes: 1024 * 1024,
    ...
    HARD_LIMITS = Object.freeze({ ...DEFAULT_LIMITS, maxDepth: 16 }), SAFE_METHODS = new Set(["GET", "HEAD"]),
    REDACTION = /^(?:<redacted>|\[redacted\])$/i, PROTOTYPE_KEY = /^(?:__proto__|prototype|constructor)$/, encoder = new TextEncoder();
function isRecord(value) { return typeof value === "object" && value !== null && !Array.isArray(value); }
function limitsFrom(options) { const out = { ...DEFAULT_LIMITS };
    if (isRecord(options)) for (const key of Object.keys(out)) if (Number.isInteger(options[key]) && options[key] > 0)
...
function addBytes(value, state, limit) {
    if (typeof value !== "string") return; const remaining = limit - state.bytes;
    if (value.length > remaining) reject("snapshot_byte_limit"); state.bytes += encoder.encode(value).length;
    if (state.bytes > limit) reject("snapshot_byte_limit");
}
function accountEntry(entry, limits, state) { if (!isRecord(entry)) return;
...
function boundedClone(value, limits, state, depth = 0) {
    if (++state.nodes > limits.maxNodes) reject("body_node_limit"); if (depth > limits.maxDepth) reject("body_depth_limit");
    if (value === null || typeof value === "boolean") return value;
    if (typeof value === "number") return Number.isFinite(value) ? value : reject("body_non_json_value");
    if (typeof value === "string") {
        ...
        return value; }
    if (Array.isArray(value)) return value.length > limits.maxArrayLength ? reject("body_collection_limit")
        : value.map((item) => boundedClone(item, limits, state, depth + 1));
    if (!isRecord(value)) reject("body_non_json_value");
    const keys = Object.keys(value).sort(compareText); if (keys.length > limits.maxObjectKeys) reject("body_collection_limit");
...
function normalizeHeaders(raw, limits) {
    if (!isRecord(raw)) return {}; const names = Object.keys(raw)
        .sort((a, b) => compareText(a.toLowerCase(), b.toLowerCase()) || compareText(a, b));
    if (names.length > limits.maxHeaders) reject("header_limit"); const out = Object.create(null);
