...
export function shapeSignature(value, options) {
    const maxDepth = limit(options, "maxDepth"), maxCollection = limit(options, "maxCollection"),
        maxVariants = limit(options, "maxVariants"), maxNodes = limit(options, "maxNodes"), active = new WeakSet(); let nodes = 0;
    function visit(node, depth) { if (++nodes > maxNodes) throw new Error("shape_work_limit");
        const kind = kindOf(node); if (kind !== "array" && kind !== "object") return kind;
        if (depth >= maxDepth) return `${kind}(*)`; if (active.has(node)) return `${kind}(cycle)`;
        active.add(node); let result; if (Array.isArray(node)) {
            if (node.length > maxCollection) result = "array(overflow)"; else {
                const variants = [...new Set(node.map((item) => visit(item, depth + 1)))].sort(compareText);
                const kept = variants.slice(0, maxVariants); if (variants.length > maxVariants) kept.push("...");
                result = `array[${kept.join("|")}]`; }
        } else { const keys = Object.keys(node); if (keys.length > maxCollection) result = "object(overflow)";
...
export function clusterResponseShapes(observations, options) {
    ...
    rows.length = Math.min(rows.length, limit(options, "maxObservations")); const counts = new Map(); for (const row of rows) { const key = JSON.stringify(row);
        counts.set(key, (counts.get(key) ?? 0) + 1); }
    return [...counts].sort(([a], [b]) => compareText(a, b)).map(([key, count]) => ({ ...JSON.parse(key), observations: count }));
}
