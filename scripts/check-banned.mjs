...
const files = execSync(
  `git diff --name-only --diff-filter=ACMR ${range} -- '*.js' '*.mjs' '*.ts' '*.tsx' '*.jsx' ` +
    `':(exclude)scripts/check-banned.mjs'`,
  { encoding: "utf8" },
)
  .trim()
...
function content(ref, path) {
  try {
    return execSync(`git show ${ref}:${JSON.stringify(path)}`, {
      encoding: "utf8",
...
for (const path of files) {
  const before = bannedCounts(content(cached ? "HEAD" : from, path), path);
  const after = bannedCounts(content(cached ? "" : "HEAD", path), path);
  for (const [label, count] of after)
    if (count > (before.get(label) ?? 0))
      failures.push(`R75 net-new banned token: ${label} (${path})`);
}
