...
const FIXTURE = JSON.parse(
    readFileSync(fileURLToPath(new URL("./fixtures/cdp-traces/truecoach-clients.json", import.meta.url)), "utf8"),
);
...
function terminalStatus(mock) {
    ...
    return status === "ingest_succeeded" || status === "ingest_failed" || status === "ingest_partial" ? status : null;
}
    ...
    it("crawls the real clients fixture, fans out to notes, ingests, and succeeds", async () => {
        ...
        global.fetch.mockImplementation(async (url, init) => {
            ...
            if (url.startsWith(CLIENTS_PREFIX) && url.includes("page=1")) {
                return { ok: true, status: 200, json: async () => ({ clients: FIXTURE_CLIENTS }) };
            }
            ...
            for (const id of CLIENT_IDS) {
                if (url === notesUrl(id)) {
                    ...
                    return { ok: true, status: 200, json: async () => ({ notes: NOTES_BY_CLIENT[id] }) };
                }
            }
            if (url === INGEST_URL) {
        ...
        for (const body of ingestBodies) {
            byType[body.entity_type] = (byType[body.entity_type] ?? 0) + body.entities.length;
        }
...
describe("makeSourceFetch — the source bearer is applied LAST (spoof resistance)", () => {
    it("drops an adapter-supplied Authorization and forces the coach's bearer", async () => {
        ...
        await fetchJson(`${SOURCE_ORIGIN}/proxy/api/clients`, {
            ...
            headers: { Authorization: "Bearer SPOOF", authorization: "Bearer spoof2", Role: "Trainer" },
            signal: null,
            timeoutMs: 1000,
        });
