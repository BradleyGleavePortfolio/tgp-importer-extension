...
describe("parseRetryAfterMs — delta-seconds form", () => {
  ...
  it("clamps a value just over the bound", () => {
    expect(parseRetryAfterMs(String(MAX_RETRY_AFTER_MS / 1000 + 1), NOW)).toBe(
      MAX_RETRY_AFTER_MS,
    );
  });

  it("accepts a value exactly at the bound", () => {
    expect(parseRetryAfterMs(String(MAX_RETRY_AFTER_MS / 1000), NOW)).toBe(
      MAX_RETRY_AFTER_MS,
    );
  });
});
...
describe("parseRetryAfterMs — HTTP-date form", () => {
  ...
  it("clamps a far-future date to the bound", () => {
    expect(parseRetryAfterMs("Tue, 28 Jul 2026 12:00:00 GMT", NOW)).toBe(
      MAX_RETRY_AFTER_MS,
    );
  });
});
