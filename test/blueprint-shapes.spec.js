...
describe("shapeSignature — stable structural identity", () => {
  ...
  it("is independent of object key insertion order", () => {
    ...
    expect(shapeSignature(left)).toBe(
  ...
  it("does not collapse unrelated schemas with equal type cardinalities", () => {
    ...
    expect(
      clusterResponseShapes([{ body: client }, { body: account }]),
    ).toHaveLength(2);
  });
  ...
  it("separates former 32-bit collisions and stays insertion-order independent", () => {
    const first = "field-1yipvhh-dnq",
      second = "field-1rrujnl-1d8i";
    expect(shapeSignature({ [first]: 1 })).not.toBe(
      shapeSignature({ [second]: 1 }),
    );
    expect(shapeSignature({ [first]: 1, [second]: "x" })).toBe(
      shapeSignature({ [second]: "y", [first]: 2 }),
    );
  });
  ...
  it("distinguishes nested member types within the depth budget", () => {
    ...
    expect(shapeSignature(strings, { maxDepth: 3 })).toBe(
      "object{client:object{enabled:boolean,id:string}}",
    );
  ...
  it("uses a stable type marker beyond the depth bound", () => {
    ...
    expect(shapeSignature(left, { maxDepth: 2 })).toBe(
      shapeSignature(right, { maxDepth: 2 }),
    );
    expect(shapeSignature(left, { maxDepth: 2 })).toBe(
      "object{client:object{profile:object(*)}}",
    );
  });
  ...
  it.each(["alice", "bob", "coachhandle", "private"])(
    "hashes untrusted-looking lowercase map key %s",
    (key) => {
      const signature = shapeSignature({ [key]: true });
      expect(signature).toMatch(/^object\{#[a-f0-9]{16}:boolean\}$/);
  ...
  it("does not expose prototype-like keys", () => {
    ...
    expect(signature).toMatch(
      /^object\{#[a-z0-9]+:number,#[a-z0-9]+:number,safe:number\}$/,
    );
  });
...
describe("shapeSignature — bounded collection work", () => {
  ...
  it("uses defaults when numeric options are invalid", () => {
    ...
    expect(
      ...
      /^object\{nested:object\{deep:object\{#[a-f0-9]{16}:string\}\}\}$/,
    );
  });
...
describe("clusterResponseShapes", () => {
  ...
  it("preserves privacy-safe field identity and child types", () => {
    expect(shapeSignature({ DanaCoach: true })).toMatch(
      /^object\{#[a-z0-9]+:boolean\}$/,
    );
    expect(shapeSignature({ DanaCoach: true, SamLift: false })).not.toBe(
      shapeSignature({ DanaCoach: true }),
    );
    expect(shapeSignature({ DanaCoach: true, SamLift: 2 })).not.toBe(
      shapeSignature({ DanaCoach: true, SamLift: false }),
    );
  });
  ...
  it.each([
    ...
    expect(signature).toMatch(
      /^object\{nested:object\{#[a-z0-9]+:object\{id:number\}\}\}$/,
    );
    expect(signature).not.toContain(key);
  });
