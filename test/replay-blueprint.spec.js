...
describe("normalizeBlueprint — budgets + rate + apiBase detail", () => {
  ...
  it.each(["maxPages", "maxPagesPerStep", "maxEntities", "requestTimeoutMs"])(
    ...
    (key) => {
      expect(
        normalizeBlueprint(base({ budgets: { [key]: HARD_BUDGETS[key] } }))
          .budgets[key],
      ).toBe(HARD_BUDGETS[key]);
      expect(
        normalizeBlueprint(base({ budgets: { [key]: HARD_BUDGETS[key] + 1 } }))
          .budgets[key],
      ).toBe(DEFAULT_BUDGETS[key]);
      expect(
        normalizeBlueprint(
          base({ budgets: { [key]: Number.MAX_SAFE_INTEGER } }),
        ).budgets[key],
      ).toBe(DEFAULT_BUDGETS[key]);
    },
  );
  it("enforces an immutable hard ceiling for rateLimitMs", () => {
    expect(
      normalizeBlueprint(base({ rateLimitMs: HARD_BUDGETS.rateLimitMs }))
        .rateLimitMs,
    ).toBe(HARD_BUDGETS.rateLimitMs);
    expect(
      normalizeBlueprint(base({ rateLimitMs: Number.MAX_VALUE })).rateLimitMs,
    ).toBe(0);
  });
...
describe("normalizeBlueprint — headers", () => {
  it.each([
    ...
    "bad@header",
    "bad[header]",
  ])("rejects RFC-forbidden header name %s", (name) =>
    expect(() =>
      normalizeBlueprint(base({ headers: { [name]: "v" } })),
    ).toThrow(/HTTP token/),
  );
