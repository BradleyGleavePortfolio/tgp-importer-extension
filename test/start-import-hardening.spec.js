...
async function settle(mock, ms = 12000) {
    ...
    for (;;) {
        ...
        const terminal = status === "ingest_succeeded" || status === "ingest_failed" || status === "ingest_partial";
...
describe("start_import — the source bearer never leaks to any surface", () => {
    it("keeps the token out of broadcasts, storage, notifications, and ingest payloads", async () => {
        ...
        global.fetch.mockImplementation(async (url, init) => {
            if (url === REFRESH_URL) return { ok: true, status: 200, json: async () => ({ access_token: "TGP-ACCESS" }) };
            if (url.startsWith(CLIENTS_PREFIX) && url.includes("page=1")) return { ok: true, status: 200, json: async () => ({ clients: [{ id: "c1" }] }) };
            if (url.startsWith(CLIENTS_PREFIX)) return { ok: true, status: 200, json: async () => ({ clients: [] }) };
            if (url === NOTES_URL) return { ok: true, status: 200, json: async () => ({ notes: [{ id: "n1" }] }) };
            if (url === INGEST_URL) { ingestBodies.push(init.body); return { ok: true, status: 200 }; }
...
describe("start_import — a degraded walk surfaces a DISTINCT partial state", () => {
    it("finalises but reports ingest_partial (not success) when a page is skipped", async () => {
        ...
        global.fetch.mockImplementation(async (url) => {
            ...
            if (url === INGEST_URL) return { ok: true, status: 200 };
            if (url === COMPLETE_URL) { completeCalls += 1; return { ok: true, status: 200 }; }
            throw new Error(`unrouted fetch ${url}`);
        });
        const ack = await mock.dispatch({ kind: "start_import", url: TAB_URL, tabId: TAB_ID });
        expect(ack).toEqual({ ok: true });
...
describe("start_import — one friendly terminal state on TGP auth loss", () => {
    it("keeps the 'session expired' state and does not overwrite it with a raw error", async () => {
        ...
        expect(snapshots(mock).at(-1).lastError).toBe("session expired — please sign in again");
        expect(statuses(mock)).not.toContain("ingest_succeeded");
        expect(snapshots(mock).some((s) => typeof s.lastError === "string" && /ingest .*401/.test(s.lastError))).toBe(false);
...
describe("single-flight guard is SHARED across start_import and start_ingest", () => {
    it("rejects a start_ingest while a start_import is in flight", async () => {
        ...
        const a = await mock.dispatch({ kind: "start_import", url: TAB_URL, tabId: TAB_ID });
    ...
    it("rejects a start_import while a start_ingest is in flight", async () => {
        ...
        const b = await mock.dispatch({ kind: "start_import", url: TAB_URL, tabId: TAB_ID });
        expect(a).toEqual({ ok: true });
        expect(b).toEqual({ ok: false, error: "import_in_progress" });
    });
