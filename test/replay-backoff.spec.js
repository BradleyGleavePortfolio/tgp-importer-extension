...
async function runWith(fetchJson, overrides = {}) {
  ...
  const result = await runReplay({
    ...
    fetchJson,
    emit: (entityType, batch) => {
      emitted.push([entityType, batch]);
    },
    sleep: (ms) => {
      slept.push(ms);
...
describe("backoffDelayMs — Retry-After takes precedence, still bounded", () => {
  it("uses the server hint instead of the exponential value", () => {
    expect(backoffDelayMs(httpError(429, { retryAfterMs: 2500 }), 1)).toBe(
      2500,
    );
  });
  ...
  it("clamps an over-large hint to MAX_BACKOFF_MS", () => {
    expect(
      backoffDelayMs(httpError(429, { retryAfterMs: 10 * MAX_BACKOFF_MS }), 1),
    ).toBe(MAX_BACKOFF_MS);
  });
  ...
  it.each([
    ...
    "ignores a %s hint and falls back to exponential backoff",
    (_label, retryAfterMs) => {
      expect(backoffDelayMs(httpError(429, { retryAfterMs }), 2)).toBe(
